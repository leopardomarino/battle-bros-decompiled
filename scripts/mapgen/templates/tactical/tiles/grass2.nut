this.grass2 <- this.inherit("scripts/mapgen/tactical_template", {
	m = {
		DetailsA = [
			"detail_grass_00",
			"detail_grass_01",
	...
	function init()
	{
		...
		t.setBrush(this.Const.Direction.N, "transition_grass_02_N");
		t.setBrush(this.Const.Direction.NE, "transition_grass_02_NE");
		t.setBrush(this.Const.Direction.SE, "transition_grass_02_SE");
		t.setBrush(this.Const.Direction.S, "transition_grass_02_S");
		t.setBrush(this.Const.Direction.SW, "transition_grass_02_SW");
		t.setBrush(this.Const.Direction.NW, "transition_grass_02_NW");
		t.setSocket("socket_earth");
		this.Tactical.setTransitions("tile_grass_02", t);
	}
	...
	function onFirstPass( _rect )
	{
		...
		tile.BlendPriority = this.Const.Tactical.TileBlendPriority.Grass2;
		...
		else
		{
			...
			else
			{
				...
				for( local allowFlowerSpecials = 0; this.Math.rand(0, 100) < this.m.ChanceToSpawnDetails && n++ < this.m.LimitOfSpawnedDetails;  )
				{
					local i = this.Math.rand(0, details.len() - 1);
					tile.spawnDetail(details[i]);

					if (i >= 10 && details == this.m.DetailsB)
					{
