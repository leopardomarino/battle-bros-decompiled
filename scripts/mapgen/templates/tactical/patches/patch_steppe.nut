this.patch_steppe <- this.inherit("scripts/mapgen/tactical_template", {
	m = {},
	function init()
	{
	...
	{
		local steppe1Tile = this.MapGen.get("tactical.tile.steppe1");
		local steppe2Tile = this.MapGen.get("tactical.tile.steppe2");
		local bumpyChance = 98;
		local mainTile = this.Math.rand(1, 100) < 50 ? steppe1Tile : steppe2Tile;
		local otherTile = mainTile == steppe2Tile ? steppe1Tile : steppe2Tile;
