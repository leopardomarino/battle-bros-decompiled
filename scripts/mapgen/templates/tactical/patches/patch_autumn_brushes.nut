this.patch_autumn_brushes <- this.inherit("scripts/mapgen/tactical_template", {
	m = {},
	function init()
	{
	...
	{
		local autumn1Tile = this.MapGen.get("tactical.tile.autumn3");
		local autumn2Tile = this.MapGen.get("tactical.tile.autumn1");
		local bumpyChance = 98;
		local mainTile = autumn1Tile;
		local otherTile = autumn2Tile;
		...
		for( local x = _rect.X; x < _rect.X + _rect.W; x = ++x )
		{
			for( local y = _rect.Y; y < _rect.Y + _rect.H; y = ++y )
			{
				...
				else
				{
					...
					if (tile.IsEmpty && this.Math.rand(1, 100) <= 20)
					{
						tile.clear();
						tile.spawnObject("entity/tactical/objects/autumn_brush");
					}
				}
			}
		}
