this.tactical_forest_leaves <- this.inherit("scripts/mapgen/tactical_template", {
	m = {},
	function init()
	{
	...
	{
		local mossTile = this.MapGen.get("tactical.tile.moss2");
		this.addRoads(_rect, _properties);
		local templates = [];
		local forestThickPatch = this.MapGen.get("tactical.patch.forest_leaves_thick");
		local forestPatch = this.MapGen.get("tactical.patch.forest_leaves");
